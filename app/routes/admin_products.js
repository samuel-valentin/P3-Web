...
router.post('/', (req, res) => {
    ...
    let x = req.body;
    const requiredFields = ['title', 'description', 'imageurl', 'unit', 'stock', 'price', 'category'];
    const missingFields = requiredFields.filter(field => x[field] === undefined);

    if (missingFields.length > 0) {
        res.status(400);
        res.send(`Error, falta agergar: ${missingFields.join(', ')}`);
    } else {
        console.log(x);
        console.log(x.title);
